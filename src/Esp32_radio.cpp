...
//*                 By Ed Smallenburg modified by Lotfi BAGHLI                                                             *
...
// GPIO18   SCK     pin 7 TFT_SCK       pin 5 SCK             CLK     -
// GPIO19   MISO    pin 9 TFT_SDO       pin 7 MISO            MISO    -
// GPIO23   MOSI    pin 6 TFT_SDI       pin 6 MOSI            MOSI    -
// GPIO15           pin 3 TFT_CS        -                     -       -
...
// 28-05-2020, Fork and add 14 bars of Spectrum Analyer, by L. BAGHLI
// 30-05-2020, added SalatTime routines: compute and display, by L. BAGHLI
//
// Define the version number, also used for webserver as Last-Modified header and to
// check version for update.  The format must be exactly as specified by the HTTP standard!
#define VERSION     "03 Juin 2020 20:08"
...
// SalatTime variables
struct FLAGS Flags;
int OldDay, OldMinute, dayMinutes, NextSalat;
...
void IRAM_ATTR timer100()
{
  ...
  {
    Flags.seconde = 1;
    if (Flags.SeqAthan)   Flags.Toggles = !Flags.Toggles;
    ...
    if ( ++timeinfo.tm_sec >= 60 )                // Yes, update number of seconds
    {
      ...
      Flags.minutes = 1;                          // ask for CheckTime() / Athan
...
void setup()
{
       ...
       ( ini_block.tft_scl_pin >= 0 ) )
  {
    ...
    if ( dsp_begin() )                                   // Init display
    {
      ...
      dsp_setCursor ( 20, 20 ) ;                           // Top of screen
      dsp_print ( "Starting..." "\n" "Version:" ) ;
      strncpy ( tmpstr, VERSION, 16 ) ;                  // Limit version length
      dsp_println ( tmpstr ) ;
      dsp_println ( "By Ed.S. and Lotfi BAGHLI" ) ;
  ...
  xTaskCreate (
    ...
    if (timeinfo.tm_year!=0) { // no Internet time
        STinit();
        Flags.SeqAthan = 0;
        Flags.AthanMP3 = 0;
        Flags.Recalcule = 1;
        Flags.displayST = 1;    // todo user choice read it
        Flags.GetNTP = 1;
    }
}
...
void chk_enc()
{
  ...
  switch ( enc_menu_mode )                                    // Which mode (VOLUME, PRESET, TRACK)?
  {
    ...
    case PRESET :
      ...
      tftset ( 3, tmp ) ;                                     // Set screen segment bottom part //todo
      break ;
    case TRACK :
    ...
    if ( datamode & DATA )                               // Test op playing
    {
      if ( av == 0 )                                     // End of mp3 data?
      {
        datamode = STOPREQD ;                            // End of local mp3-file detected
          if (Flags.AthanMP3 == 1) {
              datamode = STOPREQD;        // stop after Athan
              Flags.AthanMP3 = 0;
          }
        else {
...
void loop()
{
  ...
  mp3loop() ;                                       // Do more mp3 related actions //todo
  handlehttpreply() ;
...
const char* analyzeCmd ( const char* par, const char* val )
{
  ...
  else if (argument == "stop")                      // (un)Stop requested?
  {
      if (datamode & (HEADER | DATA | METADATA | PLAYLISTINIT |
          ...
          datamode = STOPREQD;                           // Request STOP
          Flags.displayST = 1;  // mode SalatTime
          Flags.RedrawST = 1;
      } else {
          hostreq = true;                                // Request UNSTOP
          prevbands = 0;    // force erase
          Flags.displayST = 0;  // mode Spectrum
      }
  }
  else if (argument == "st")                      // salatTime ou Spectrum
  {
      if (ivalue == 1) {
          Flags.displayST = 1;  // mode SalatTime
          Flags.RedrawST = 1;
   ...
   else if (argument == "tst")                      // salatTime ou Spectrum
  {
      if (ivalue == 0) { //fajr
          ...
          timeinfo.tm_min = 06;
          timeinfo.tm_sec = 55;
          // http://192.168.1.20/index.html/?tst=1
      }
      if (ivalue == 1) {//Zuhr
...
void Forcegettime()
{
    dbgprint ( "Sync TOD after midnight" ) ;
    if ( !getLocalTime ( &timeinfo ) )                    // Read from NTP server
    {
        dbgprint ( "Failed to obtain time!" ) ;             // Error
        timeinfo.tm_year = 0 ;                              // Set current time to illegal
...
bool handle_tft_txt()
{
    if ( tftdata[2].update_req && Flags.displayST)  Flags.RedrawST = 1;
...
void displayTimeLine()
{
    ...
    if (mp3filelength > 2048 )
    {
        if (timeLineFilePlayed > 2048 )
        {
            played1 = (150*(timeLineFilePlayed - 2048))/(mp3filelength - 2048);
            if (played1 < played)
            {
...
void displaySpectrum() {
    if (Flags.displayST) return;    // no space for bars
...
//    }
    if (bands != prevbands) {
        prevbands = bands;
    if (visual)
        dsp_fillRect (0, Spectrum_y0, dsp_getwidth(), Spectrum_hy + 8, BLACK);
    }
...
void handle_spec()
{
  ...
  if ( time_req )                                             // Time to refresh timetxt?
  {
    ...
    if ( NetworkFound  )                                      // Time available?
    {
      ...
      if (Flags.SeqAthan || Flags.RedrawST)  displaySalatTime();
    }
  }
  releaseSPI() ;                                              // Release SPI bus
  if ( mqtt_on )
...
//                                     displaySalatTime                                            *
//**************************************************************************************************
// Handles display of alatTime and Date on TFT.                                                    *
// This task runs on a low priority.                                                               *
//**************************************************************************************************
void displaySalatTime() {
    if (!tft )  return;
    if (!Flags.displayST) return;
    char S[100];
    dsp_fillRect (0, Spectrum_y0, dsp_getwidth(), Spectrum_hy + 1, BLACK);
    dsp_setTextColor ( CYAN ) ;
    if (timeinfo.tm_year==0)    { // No Time/date
        dsp_setCursor ( 25, Spectrum_y0 +20);
        dsp_print ("No Time from Internet");
        Flags.RedrawST = 0;
        return;
        }
    // http://www.cplusplus.com/reference/ctime/strftime/
    strftime(S, 50, "%a %d/%m/%Y", &timeinfo);
    dsp_setCursor ( 70, Spectrum_y0);
    dsp_print (S);

    sprintf(S, "Fajr %02hhu:%02hhu", SalatT.h[0], SalatT.m[0]);
    dsp_setCursor ( 98, Spectrum_y0 +20);
    dsp_print (S);

    sprintf(S, "Zuhr %02hhu:%02hhu", SalatT.h[1], SalatT.m[1]);
    dsp_setCursor ( 25, Spectrum_y0 +40);
    dsp_print (S);

    sprintf(S, "Asr %02hhu:%02hhu", SalatT.h[2], SalatT.m[2]);
    dsp_setCursor ( 192, Spectrum_y0 +40);
    dsp_print (S);

    sprintf(S, "Magrib %02hhu:%02hhu", SalatT.h[3], SalatT.m[3]);
    dsp_setCursor ( 1, Spectrum_y0 +60);
    dsp_print (S);

    sprintf(S, "Isha %02hhu:%02hhu", SalatT.h[4], SalatT.m[4]);
    dsp_setCursor ( 180, Spectrum_y0 +60);
    dsp_print (S);

    if (Flags.SeqAthan) {
        if (Flags.Toggles) dsp_setTextColor (RED);
        else dsp_setTextColor (GREEN);
        dsp_setCursor (0, Spectrum_y0);
        dsp_print ("Athan"); // todo clignote le Athan en question
        switch (NextSalat) {
            case 0 : // Fajr
                sprintf(S, "Fajr %02hhu:%02hhu", SalatT.h[0], SalatT.m[0]);
                dsp_setCursor ( 98, Spectrum_y0 +20);
    ...
    Serial.println("displaySalatTime");
    Flags.RedrawST = 0;
}
...
//                                     CheckTime                                                   *
//**************************************************************************************************
// Check if Athan or not                                                                           *
// This task runs on a low priority.                                                               *
//**************************************************************************************************
void CheckTime() {
    Flags.seconde = 0;
    char S[50]; //50 chars should be enough
    strftime(S, 50, "CheckTime %d/%m/%Y %H:%M:%S", &timeinfo);
    Serial.println(S);

    Flags.minutes = 0;
    if (OldDay != timeinfo.tm_mday) Flags.Recalcule = 1;  // recalcule ST
    if ((OldMinute != timeinfo.tm_min) && (Flags.Recalcule == 0)) {
        // une minute d'est écoulée
        if ((SalatT.m[NextSalat] == timeinfo.tm_min)
            && (SalatT.h[NextSalat] == timeinfo.tm_hour)) {// Athan
            Flags.SeqAthan = 1;
            playAthan();
        } else {
            Flags.SeqAthan = 0;
        ...
        OldMinute = timeinfo.tm_min;
        dayMinutes = timeinfo.tm_hour * 60 + timeinfo.tm_min;
        for (int i = 4; i >= 0; i--)
            if (SalatT.h[i] * 60 + SalatT.m[i] >= dayMinutes) {
                NextSalat = i;
                sprintf(S, "SalatT.m[%d] = %d", i, SalatT.m[i]);
                Serial.println(S);
            }
    }
}
...
void playAthan() {
    ...
    host = "localhost/azan.mp3";       // Save it for storage and selection later
    hostreq = true;                                  // Force this station as new preset
    Flags.AthanMP3 = 1;
}
